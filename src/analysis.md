...
6. Выполнение простых воспроизводимых сценариев через `script`.
...
- запрашивает подтверждение изменяющих операций с пакетами, компонентами, службами и источниками ПО;
...
- `script` другого файла с ограничением глубины;
- номера файла и строки в диагностике;
- явную политику остановки после ошибки.

```text
actl> script --stop configure.actl
actl> script --continue inspect.actl
```
...
Интерактивный `script` запрашивает подтверждение.

Ограничения:

- обнаружение рекурсивных вызовов `script`;
...
| Сценарии           | Внешний Bash скрипт                   | Встроенное выполнение файлов через `script` |
...
- `history save` и `script` с комментариями, номерами строк и `stop-on-error`;
